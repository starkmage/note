...
可以把整个加密过程想象成一个非常精妙的“密钥交换仪式”，这个过程被称为 **TLS/SSL 握手（Handshake）**。

------

### TLS 握手：加密的完整流程

整个 HTTPS 加密过程就是结合了以上两种加密方式的 TLS 握手：

1. **客户端问好 (Client Hello)**：浏览器向服务器发送它支持的加密算法列表。
2. **服务器问好 (Server Hello)**：服务器从列表中选择一个它支持的加密算法，并把自己的 **TLS 证书**（内含公钥）发送给浏览器。
3. **身份验证 (Authentication)**：浏览器验证服务器的证书是否可信。如果证书有效，浏览器就信任了服务器。
4. **密钥交换 (Key Exchange)**：浏览器生成一个**会话密钥**，并用服务器的公钥加密后发送给服务器。只有服务器的私钥能解密它。
5. **安全通信 (Secure Communication)**：现在浏览器和服务器都拥有了相同的会话密钥。从这一刻起，所有数据都将用这个密钥进行对称加密和解密，实现高效且安全的通信。

通过这个复杂的握手过程，HTTPS 确保了：

- **身份认证**：你访问的网站是真实的，而不是被伪装的。
- **数据完整性**：传输中的数据没有被篡改。
- **端到端加密**：所有数据都是加密传输的，即使被截获也无法阅读。
