| 名称      | 所属层级   | 定义来源          | 是否原生 | 初次出现            |
| --------- | ---------- | ----------------- | -------- | ------------------- |
| **XHR**   | 浏览器 API | XMLHttpRequest    | ✅ 是     | 2000 年左右（很老） |
| **fetch** | 浏览器 API | Fetch API（ES6+） | ✅ 是     | 2015（现代）        |
| **axios** | 第三方库   | 基于 XHR 或 fetch | ❌ 不是   | **社区封装库**      |

| 特性/功能         | `fetch`                              | `XMLHttpRequest`（XHR）      | `axios`                        |
| ----------------- | ------------------------------------ | ---------------------------- | ------------------------------ |
| 是否原生支持      | ✅ 现代浏览器原生                     | ✅ 所有浏览器支持             | ❌ 需要引入第三方库             |
| 是否基于 Promise  | ✅ 是（内建支持）                     | ❌ 否（基于回调）             | ✅ 是（基于 Promise 封装）      |
| 请求取消          | ❌ 需配合 `AbortController`           | ✅ 可 `xhr.abort()`           | ✅ 支持（内建取消令牌）         |
| 请求/响应拦截器   | ❌ 无                                 | ❌ 无                         | ✅ 内建支持                     |
| 自动 JSON 解析    | ❌ 无（需手动 `res.json()`）          | ❌ 无（需 `JSON.parse`）      | ✅ 自动处理                     |
| 上传/下载进度监控 | ❌ 较难                               | ✅ 支持                       | ✅ 支持（基于 XHR）             |
| 跨域 cookie 支持  | ❌ 默认不带（需手动加 `credentials`） | ✅ 可配置 `withCredentials`   | ✅ 默认处理好                   |
| 兼容性            | ✅ 现代浏览器（IE 不支持）            | ✅ 全浏览器支持（含 IE）      | ✅ 内部兼容处理（适配旧浏览器） |
| 错误处理          | ❌ 仅网络错误进 catch                 | ✅ 请求失败或状态码均回调触发 | ✅ 统一封装                     |
| 请求超时          | ❌ 不支持（需自实现）                 | ✅ 支持                       | ✅ 支持                         |
| 体积/依赖         | 零依赖                               | 零依赖                       | 需要引入库（~15KB+）           |
